...
1. **Via UI** — Use the "按键配置" (Key Config) menu in KOReader to view, add, edit, or delete mappings. Changes are saved to `settings/kindlebtcontroller.lua` and persist across restarts.
...
After installation, find "蓝牙控制器" (Bluetooth Controller) in the KOReader menu under device settings.

#### Menu Options

- **蓝牙开关** — Toggle Bluetooth on/off
- **已连接设备** — Shows the currently connected device name
- **重载设备** — Manually reload the Bluetooth input device
- **按键检测** — Enter key detection mode to identify button codes
- **按键配置** — View and edit all key mappings

#### Key Detection Mode

1. Open "按键检测" from the menu
...
- **Buttons not working**: Use "按键检测" to verify button codes, then check mappings in "按键配置".
